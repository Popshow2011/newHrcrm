<template>
  ...
  >
    <div v-for="(item, i) in storeLanguage" :key="item.id">
...
import LanguageItem from 'Elements/LanguageForm/LanguageItem';
...
export default {
  ...
  methods: {
    setValue(item, index, fields = {}) {
      const language = [...this.storeLanguage.slice()];

      for (let name in fields) {
        const value = fields[name];

        language[index].fields = item.fields.map(field =>
    ...
    addLanguage() {
      store.dispatch('common/getAddLanguage');
      this.validateForm();
    },
    validateForm() {
      let success = '';
  ...
  components: { 'el-form': Form, LanguageItem, Button }
};
</script>
